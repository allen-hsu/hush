...
# deny_agent_run = true     # 偵測到 agent 時連 hush run 也拒絕 —— 只給人類跑
# agent_profile = "sandbox" # 偵測到 agent 時改用這個 profile —— 放測試憑證讓它能跑但看到假值
```
...
**Per-project agent 策略**（寫在 `.hush.toml`）：

- `disable_get = true` —— 連人都不給 `hush get`（值只能透過 `run` 用）。
- `deny_agent_run = true` —— 偵測到 agent marker 時連 `hush run` 也拒絕，老實的 agent 完全拿不到值（由人類跑需要密鑰的命令）。
- `agent_profile = "sandbox"` —— 偵測到 agent 就改用 sandbox profile，讓它**仍然能跑程式**但只看得到丟棄式測試憑證。通常最實用：別再對 agent 藏 secret，給它一組無所謂的。（此時 `extends` 會關閉，避免缺 key 時從 `base` 漏出真值。）

這些是**對老實/粗心 agent 的護欄，不是牆**：偵測靠 marker，同 uid 的程式可以 `unset` 掉 marker（或照抄包裝腳本的 `unset`）來繞過。同一個 uid 下,你**無法**既讓 agent *跑*帶 secret 的 process、又阻止它讀那個 process 自己的 env/記憶體。真正的邊界要把特權執行放到 agent 控制不到的地方:另一個 user、一個只回結果的 daemon/broker、或把 app 當服務跑讓 agent 只透過 localhost 互動。純 `.env` 比這些都弱——agent 直接讀檔就好。
