...
```sh
hush run -- forge script ...     # 解密、把 env 只注入這個子行程、然後 exec
```
...
hush run -- forge script ... # 解密、把 env 只注入這個子行程、exec
```
...
- **shim**：對 `.hush.toml` `shims = [...]` 裡的每個命令（你自己挑），打裸命令會自動包起來——
  `forge ...` 實際以 `hush run -- forge ...` 執行。值只進那個子行程，你的 shell 保持乾淨。
...
shims   = ["forge", "cast"] # opt-in；要自動包 hush run 的命令
```
